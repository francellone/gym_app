...
| `v25e` | Refactor de los triggers v25d al patrón **upsert** (UPDATE body si el mirror existe, INSERT si no) en lugar de soft-delete+insert. Preserva el id de la nota entre ediciones, lo que (a) permite mostrar el indicador "· editada", (b) evita que la burbuja "salte" de posición en el listado, y (c) habilita que el frontend rute las ediciones de notas mirror al campo legacy para que el alumno pueda editar sus comentarios viejos desde el panel. | ✓ aplicada |
| `v25f` | Bug fix crítico: el trigger genérico `notes_updated_at` se disparaba en **cualquier** UPDATE (incluyendo `markThreadRead`, `softDeleteNote` y el propio backfill v24c), pisando `updated_at` y haciendo que todas las 67 notas existentes apareciesen falsamente como "· editadas". Restringido a `BEFORE UPDATE OF body, tags, visibility, context_type, context_id, parent_note_id` (solo cambios de contenido real). Reset masivo de `updated_at = created_at` para las 67 notas afectadas. | ✓ aplicada |
| `v26a` | **Fase D step 1.** Trigger `fn_sync_profile_observations_to_notes` AFTER INSERT OR UPDATE OF `observations`, `coach_notes` en `profiles`. Cada save en StudentInfoTab que cambia uno de los dos campos crea una nueva nota en el thread del alumno (patrón INSERT/historia, no upsert). Cierra el agujero donde el coach editaba observaciones y los cambios no aparecían en el panel. | ✓ aplicada |
| `v26b` | **Cierra deuda session_day (Opción B).** Agrega columna `notes.note_date date` para "sobre qué día habla la nota" (separado de `created_at` = "cuándo se escribió"). Trigger respeta `note_date` manual cuando `context_type='free'`, lo deriva de `workout_logs.logged_date` para mirrors. Backfill aplicado: 60 workout_log + 1 workout_block_log con fecha real. Composer suma la 4ta solapa "Día" con date picker. NoteCard muestra "Día: dd/mm/yyyy" para notas free + note_date. | ✓ aplicada |
| `v26c` | **Cierra deuda D5.** `fn_sync_eval_response_to_notes` ahora usa `NEW.id` (response.id) como `context_id` en lugar de `NEW.test_id`. Mapping 1:1 nota↔response, sin ambigüedad cuando un alumno tiene múltiples evaluation_results del mismo test. `notes_resolve_context` para `evaluation_test` ahora hace `JOIN etr → et` para resolver `exercise_id`. Frontend habilita editar/borrar notas eval desde el panel rutando a la columna correspondiente de evaluation_test_responses (`student_comment`, `coach_comment_public` o `coach_comment_private`) según `author_role + visibility`. | ✓ aplicada |
| `v26d` | **Cierre del refactor (1ra pasada).** Backup completo a `schema=archive` (4 profiles, 59 workout_logs, 1 workout_block_log, 0 eval). Drop de los 3 triggers de sync (`fn_sync_workout_log_to_notes`, `fn_sync_eval_response_to_notes`, `fn_sync_profile_observations_to_notes`). RPC `save_workout_log` actualizado: parámetro `p_notes` queda por compat de bundles viejos pero se ignora. DROP COLUMN de las 7 columnas legacy: `profiles.observations`, `profiles.coach_notes`, `workout_logs.notes`, `workout_block_logs.notes`, `evaluation_test_responses.{student_comment, coach_comment_public, coach_comment_private}`. | ✓ aplicada |
| `v26e` | Fix: trigger `notes_reset_read_on_body_edit` BEFORE UPDATE OF body resetea `read_at_<receptor>` para que la nota se vea como nueva al receptor tras una edición. `notes_bump_thread` actualizado para manejar el caso "re-unread" (`read_at` pasando de timestamp a NULL) y para bumpear `last_message_at` en edits. | ✓ aplicada |
| `v26f` | **🎯 CIERRE FINAL del refactor.** Caso descubierto post‑v26d: si un alumno guarda una eval custom sin pruebas configuradas, su "Observación general" iba a la columna `evaluation_results.notes`, que no estaba en el scope. v26f suma `'evaluation_result'` al enum de `context_type`, agrega rama en `notes_resolve_context` (`note_date = eval_date`), backfill de notas existentes, backup a archive y DROP COLUMN. Frontend: `postEvalResultNote` helper nuevo, `EvalWorkoutPage` save sin la columna + reads desde el panel, `EvaluationDetailPage` mergea body del panel sobre `results.notes` (jsonb queda como fallback). La tabla `notes` ahora es fuente única de verdad para los 8 campos legacy migrados. | ✓ aplicada |
...
- ~~🟡 DEFECT **D5 (`evaluation_test` con `context_id = test_id`)**~~ → **Cerrado por v26c**.
- ~~🔵 DRIFT **DR6 (`notes_resolve_context` no maneja `session_day`)**~~ → **Cerrado por v26b** (Opción B con `note_date` reemplaza la necesidad de un handler `session_day`).
...
- **`workout_sessions.{day}_notes`**: **DEUDA FANTASMA — ya no existe.** Investigación 2026‑05‑17: la tabla `workout_sessions` actualmente tiene solo 10 columnas (`id, student_id, plan_id, logged_date, started_at, finished_at, created_at, updated_at, logged_late, borg_per_day jsonb`). Las "7 columnas día‑por‑día" fueron consolidadas a `borg_per_day` por una migración previa al refactor de notas (probablemente `_modificaciones/migration_borg_per_day.sql`). No queda nada que dropear.

- ~~**Bug latente descubierto en `TodayWorkoutPage.saveDayPSE`**~~ — **Cerrado el 2026‑05‑17.** Corrección de diagnóstico previo: el código NO fallaba con 42703 — guardaba la observación como key `${day}_notes` *dentro* del jsonb `borg_per_day` (válido para Postgres). El bug real era que esas keys quedaban enterradas en el jsonb sin que ningún UI las leyera. Fix aplicado: (a) saqué la línea `${day}_notes`; (b) si `effortNotes` viene con texto, ahora se llama `postPSEDayNote()` (nuevo helper en `lib/notes.js`) que publica la observación como nota libre en el panel del coach‑alumno, con `note_date = workout_sessions.logged_date` y body prefijado `[Día A] ...` para distinguir la sección. El PSE numérico sigue en `borg_per_day`. El error del posteo no rompe el flujo del modal — el efforto/scale se guarda siempre.
...
**Regla actualizada con v25e**: las notas con context_type `workout_log` y `workout_block_log` ahora **también son editables/borrables** desde el panel (siempre que la nota sea propia). El frontend rutea automáticamente: cuando el alumno edita una nota mirror desde el panel, llamamos `UPDATE workout_logs SET notes='nuevo body'`, y el trigger v25e (upsert) actualiza el mirror in‑place preservando el id. Sigue siendo read‑only `evaluation_test` (deuda D5: ambigüedad context_id) y `plan_exercise` (no se crea desde UI).
