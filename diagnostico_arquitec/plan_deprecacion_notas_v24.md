...
- **Bug latente descubierto en `TodayWorkoutPage.saveDayPSE` (línea 1567)**: el código del front sigue intentando `UPDATE workout_sessions SET ${day}_notes = effortNotes` apuntando a columnas que ya no existen. Postgres rechaza con error 42703, el front loggea `console.error` (línea 1592) pero el usuario no se entera. **Resultado: cada vez que un alumno completa el modal PSE diario con texto, esa observación se pierde silenciosamente**. Fix posible (separate PR): (a) sacar la línea broken; (b) si `effortNotes` viene seteado, llamar `createNote` con `context_type='free'`, `note_date = workout_sessions.logged_date`, body con prefijo `[Día A]` (o equivalente) que indique la sección. Sin migración necesaria — v26b ya proveyó la infra. Estimado ~30min. Diferido a confirmación del usuario.
