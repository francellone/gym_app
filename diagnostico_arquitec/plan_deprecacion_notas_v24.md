...
- **`workout_sessions.{day}_notes`**: 7 columnas día‑por‑día con notas del PSE. ~~Requiere decidir~~ **Decisión tomada (v26b, Opción B)**: usar `context_type='free'` con `note_date` manual; descartado FK a `workout_sessions.id`. Para deprecar estas 7 columnas falta: (a) trigger `fn_sync_session_day_notes_to_notes` que espeje cada `monday_notes`/`tuesday_notes`/... → nota free con `note_date = (week + offset)`, (b) actualizar `TodayWorkoutPage.saveDayPSE` para que el panel los recoja, (c) migration v26c que dropea las 7 columnas. Tarea diferida pero ya no tiene bloqueo de schema.
