...
- **F1** — notif al dashboard cuando el alumno cumple la evaluación. **RESUELTO 30/05.** Backend: se extendió `fn_close_eval_on_result` (trigger AFTER INSERT en `evaluation_results`) para, además de cerrar la asignación, insertar `notifications(type='evaluation_completed')` al coach (resuelto vía `profiles.coach_id`, mismo patrón que `fn_notify_session_completed`), con dedup por alumno+plan+día y título del **template** (no del clon). **Gotcha crítico que atrapó el smoke SQL:** el CHECK `notifications_type_check` no incluía el tipo nuevo → al ser AFTER INSERT habría abortado la carga del `evaluation_result` entero; se amplió el constraint de 12→13 tipos (migración aparte). Front: `getNotificationTargetUrl` rutea `evaluation_completed` → `/coach/students/{id}?tab=evaluaciones` + icono + test. Smoke DB con rollback: notif creada, coach correcto, título limpio. 272/272 tests, lint 0 err, build OK. **Migraciones vivas en prod; falta push del front.**
