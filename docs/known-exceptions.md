...
### `public.intake_profile_snapshots` es snapshot inmutable del intake, NO source-of-truth

- **Síntoma**: existe una tabla `public.intake_profile_snapshots` (4 filas) con columnas que parecen "perfil del alumno" (`objetivo_principal`, `nivel_experiencia`, `frecuencia_semanal`, `lugar_entrenamiento`, `tiene_lesiones`, `patologias`, `nombre`, `apellido`). El nombre ya te avisa, pero por las dudas: **no leerla en features nuevas**.
...
- **Mitigación / defensa**: el nombre de la tabla cuenta su rol. Además tiene `COMMENT ON TABLE` explícito + `COMMENT ON COLUMN` en `objetivo_principal`, `nivel_experiencia`, `frecuencia_semanal`, `raw_data`, `submission_id` que apuntan a sus equivalentes operacionales. Cualquier `\d+ public.intake_profile_snapshots` en psql o introspección por dashboard lo muestra. Si abrís un PR que lea de esta tabla, asumí que estás en el camino equivocado y mirá `public.profiles` primero.
...
- **Historia**: vivía como `archive.student_profiles` hasta 2026-05-23 AM (handoff 16). Movida a `public.student_profiles` con COMMENTs. Renombrada a `intake_profile_snapshots` ese mismo PM para que el nombre cuente la historia sin necesidad de leer COMMENTs.
...
- **Resolución**: dos migraciones encadenadas el 2026-05-23 (handoff 16):
  1. `move_student_profiles_to_public_with_clarifying_comments` (AM) — `ALTER TABLE archive.student_profiles SET SCHEMA public` + `COMMENT ON TABLE` + `COMMENT ON COLUMN` en 5 columnas. Policies, FKs, trigger y RLS viajaron con la tabla automáticamente.
  2. `rename_student_profiles_to_intake_profile_snapshots` (PM) — `ALTER TABLE ... RENAME TO intake_profile_snapshots` + rename de policies, trigger y 4 constraints para coherencia de nombres. Final state: nombre semántico que describe el rol sin necesidad de leer COMMENTs.
- **Hallazgo en el diagnóstico previo**: la tabla estaba huérfana (0 funciones Postgres la referencian, 0 archivos del front la referencian). NO era operacional como se asumía en el doc 11 §2.1 — era residuo del flujo original del intake que `process_intake_submission` ya no usa (proyecta directo a `public.profiles`). Esto bajó el riesgo del move + rename a casi cero (sin refactor en front).
- **Defensa contra recaída**: ver entrada vigente "`public.intake_profile_snapshots` es snapshot inmutable del intake, NO source-of-truth" más arriba.
