-- 20260526150200_assign_template_clones_with_cloned_from.sql
-- ===========================================================
-- C1 del doc 34 (2026-05-26 PM) — parte runtime:
--   Recrear assign_template_to_student para que el clon nuevo
--   registre cloned_from_plan_id = p_template_id. Cierra la
--   trazabilidad formal template→clon que C1 pide.
--
--   Resto de la función queda idéntica a la versión del
--   2026-05-26 (migración 20260526120000) — solo se agrega
--   `cloned_from_plan_id` al INSERT del plans.
-- ===========================================================
...
BEGIN
  ...
  -- Clon del plan. cloned_from_plan_id apunta al template original
  -- (C1 doc 34). parent_plan_id queda NULL (reservado para linkeo
  -- eval-template→training-template, ver trigger plans_validate_parent).
