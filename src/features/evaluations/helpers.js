...
export function calc1RM(method, weight, reps) {
  ...
  switch (method) {
    ...
    case 'lander':
      return +((100 * w) / (101.3 - 2.67 * r)).toFixed(1)
    case 'lombardi':
      return +(w * Math.pow(r, 0.1)).toFixed(1)
    case 'mayhew':
      return +((100 * w) / (52.2 + 41.9 * Math.exp(-0.055 * r))).toFixed(1)
    default:
      return null
  }
}
...
export function calcVO2max(method, inputs) {
  ...
  switch (method) {
    ...
    case 'rockport': {
      ...
      return +(132.853 - 0.0769 * wLb - 0.3877 * a + sexFactor - 3.2649 * t - 0.1565 * hr).toFixed(
        1
      )
    }
    case 'yoyo': {
...
export function calcBodyComp(method, inputs) {
  ...
  switch (method) {
    ...
    case 'navy': {
      ...
      let fat_pct
      if (isMale) {
        fat_pct = +(86.01 * Math.log10(waist - neck) - 70.041 * Math.log10(h) + 36.76).toFixed(1)
      } else {
        fat_pct = +(
...
export function calcFMSScore(patterns) {
  ...
  const asymmetries = patterns.filter(
    (p) => p.bilateral && p.score_left !== p.score_right && !p.pain
  )
...
export function emptyResults(evalType, method) {
  switch (evalType) {
    case 'one_rm':
      return {
        method: method || 'brzycki',
        exercises: [{ name: '', weight_kg: '', reps: '', one_rm: null }],
        notes: '',
      }
    case 'max_reps':
      return {
        method: method || 'pushup',
        reps: '',
        weight_kg: '',
        volume: null,
        time_sec: '',
    ...
    case 'scored':
      return {
        ...
        reach_anterior_l: '',
        reach_anterior_r: '',
        reach_posteromedial_l: '',
        reach_posteromedial_r: '',
        reach_posterolateral_l: '',
        reach_posterolateral_r: '',
        result: null,
        notes: '',
      }
    case 'custom':
