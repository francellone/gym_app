...
const TRIGGER_OPTIONS = [
  ...
  { id: 'on_plan_end',  label: 'Al cierre del plan',    hint: 'Se envía cuando termine el plan asignado' },
]
...
export default function SendToStudentModal({
  ...
  // ── Helpers ─────────────────────────────────────────────
  const triggerNeedsPlan = isFollowUp && (triggerType === 'on_week' || triggerType === 'on_plan_end')
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-400 mt-0.5 truncate">
              {isFollowUp && templateName ? templateName : 'Seleccioná los alumnos que recibirán este formulario'}
            </p>
          </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-semibold text-green-800">
                    {triggerType === 'manual' ? 'Enviado' : 'Programado'} para {results.sent.length} alumno{results.sent.length !== 1 ? 's' : ''}
                  </span>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                ...
                {TRIGGER_OPTIONS.map(opt => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Calendar size={14} className={triggerType === opt.id ? 'text-blue-600' : 'text-gray-400'} />
                ...
                {triggerType === 'on_week' && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mt-2">
                    <label className="block text-xs font-medium text-blue-800 mb-1.5">Semana del plan</label>
                    <input
                      type="number"
                      min={1}
                      max={52}
                ...
                <div className="flex-1 overflow-y-auto p-2">
                  {loading ? (
                    ...
                    filteredStudents.map(student => {
                      ...
                      const needsChoice = triggerNeedsPlan && studentPlans.length > 1 && selected && !planChoice[student.id]
                        ...
                        <div key={student.id}>
                          ...
                          {needsChoice && (
                            <div className="ml-8 mb-2 bg-amber-50 border border-amber-200 rounded-lg p-2 space-y-1">
                              <p className="text-xs text-amber-800 font-medium">Elegí el plan para este trigger:</p>
                              {studentPlans.map(pa => (
                                <button
                                  key={pa.id}
                                  onClick={() => setPlanChoice(prev => ({ ...prev, [student.id]: pa.id }))}
                                ...
                                >
                                  {pa.plans?.title || 'Plan'} {pa.start_date && `(desde ${pa.start_date})`}
                                </button>
                              ))}
                            </div>
                          )}
                ...
                <div className="p-4 border-t border-gray-100 space-y-2">
                  {studentsNeedingPlanChoice.length > 0 && (
                    <p className="text-xs text-amber-700 text-center">
                      Falta elegir plan para {studentsNeedingPlanChoice.length} alumno{studentsNeedingPlanChoice.length !== 1 ? 's' : ''}
                    </p>
                  )}
